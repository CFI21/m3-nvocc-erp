...
from app.clx032_finance_controls import (
...
def isolated_db(tmp_path, monkeypatch):
    monkeypatch.setattr(db, 'DB_PATH', tmp_path / 'clx032.db')
    admin_seed_run()
    c=db.connect()
    ...
    c.execute(
        '''INSERT INTO iam_user_roles(user_id,role_id,office_id,valid_from,status,assigned_by)
           VALUES(?,?,?,?,?,?)''',
        (uid,role,office,'2026-09-26T00:00:00+00:00','ACTIVE','seed-clx032'),
    )
    c.close()
    return db.DB_PATH
...
def approve_review(review_ref,checker):
    return decide(review_ref,Decision(decision='APPROVE',comment='Independent CLX-032 approval'),checker)
